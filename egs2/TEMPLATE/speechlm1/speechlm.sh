...
skip_test=false     # Skip test prep stages.
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        if ${use_multistream}; then
            _opts+="--use_multistream true "
        fi
        if ${use_multistream_subtask}; then
            _opts+="--use_multistream_subtask true "
        fi
        if ${use_multistream_2tasks}; then
            _opts+="--use_multistream_2tasks true "
        fi
        if ${use_spk_count}; then
            _opts+="--use_spk_count true "
        fi
        if ${use_spk_count_after}; then
            _opts+="--use_spk_count_after true "
        fi
        if ${use_spk_count_after1}; then
            _opts+="--use_spk_count_after1 true "
        fi
        if ${use_sc_token}; then
            _opts+="--use_sc_token true "
        fi
        if ${use_sc_token_2types}; then
            _opts+="--use_sc_token_2types true "
        fi
        if ${use_sc_token_1type}; then
            _opts+="--use_sc_token_1type true "
        fi
        ...
        if ${use_sad_after}; then
            _opts+="--use_sad_after true "
        fi
        if ${use_od_after}; then
            _opts+="--use_od_after true "
        fi
        if ${use_od_before}; then
            _opts+="--use_od_before true "
        fi

        if ${use_timestamp}; then
            _opts+="--use_timestamp true "
        fi

        if ${use_local_speaker}; then
            _opts+="--use_local_speaker true "
        fi

        if ${use_dur_format}; then
            _opts+="--use_dur_format true "
        fi

        if ${use_padding}; then
            _opts+="--use_padding true "
        fi

        if ${skip_train}; then
            _dsets=("${test_sets}")
        elif ${skip_test}; then
            _dsets=("${train_set} ${valid_set}")
        else
        ...
        for dset in ${_dsets}; do
            for data_output in ${data_outputs}; do
                local/data.sh \
                    ...
                    --rttm_file ${data_dir}/${dset}/sorted.rttm \
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        if ${skip_train}; then
            ...
            _dsets="${valid_set} ${test_sets} ${train_set}"
        fi
        ...
        for dset in ${_dsets}; do
            ...
            for triplet in ${all_triplets}; do
                ...
                if [ ${_modality} == "codec" ] || [ ${_modality} == "ssl" ] || [ ${_modality} == "codec_ssl" ]; then
                    # Format audio
                    _opts=
                    if [ ${data_name} == "ami" ] || [ ${data_name} == "alimeeting" ] || [ ${data_name} == "aishell4" ]; then
                        if ${use_random_durs}; then
                            log "${data_dir}/"${dset}"/segments_random_dur_30_20_10"
                            if [ -e ${data_dir}/"${dset}"/segments_random_dur_30_20_10 ]; then
                                _opts+="--segments ${data_dir}/${dset}/segments_random_dur_30_20_10 "
                            fi
                        else
                            log "${data_dir}/"${dset}"/segments_dur${dur}_skip${skip}"
                            if [ -e ${data_dir}/"${dset}"/segments_dur${dur}_skip${skip} ]; then
                                _opts+="--segments ${data_dir}/${dset}/segments_dur${dur}_skip${skip} "
                            elif [ -e ${data_dir}/"${dset}"/segments_dur${dur}_skip${skip}_context${context_length} ]; then
                                _opts+="--segments ${data_dir}/${dset}/segments_dur${dur}_skip${skip}_context${context_length} "
                            fi
                        fi
                    fi
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        for dset in ${_dsets}; do
            ...
            for triplet in ${all_triplets}; do 
                ...
                elif [[ ${_modality} == "diar_tokenizer" || ${_modality} == "diar_tokenizer_multistream" ]]; then                    
                    log "Use ${_modality}"
                    log "${diar_token_list}"
                    log "${data_feats}/${dset}/token_lists/${_modality}_token_list"
                    cp "${diar_token_list}" "${data_feats}/${dset}/token_lists/${_modality}_token_list"
                    cp ${data_audio}/${dset}/${_name} ${data_feats}/${dset}/${_name}
            ...
            # cp ${data_audio}/${dset}/wav.scp ${data_feats}/${dset}/wav.scp 
...
if ! "${skip_eval}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        for test_json in ${test_jsons}; do
            ...
            if [[ ${data_name} == "alimeeting" || ${data_name} == "aishell4" ]]; then                    
                _dir="${speechlm_exp}/${inference_tag}/${task}_${data_name}_${dset}";
            else
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        for test_json in ${test_jsons}; do
            ...
            if [[ ${data_name} == "alimeeting" || ${data_name} == "aishell4" ]]; then                    
                _dir="${speechlm_exp}/${inference_tag}/${task}_${data_name}_${_dset}";
            else
            ...
            if ${check_overlap_sad}; then
                _opts+="--check_overlap_sad true "
            fi

            if ${check_overlap_od}; then
                _opts+="--check_overlap_od true "
            fi

            if ${apply_local_speaker_matching}; then
                _opts+="--apply_local_speaker_matching true "
            fi
