...
valid_set="dev"
test_set="test"
config_name="train_delay_pretrain_lr1e-4_warmup4k"
...
dump_dir="dump_alimeeting"
exp_dir="exp_alimeeting"
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ./speechlm.sh \
    ...
    --cmd_backend "slurm" \
    --speechlm_exp "${exp_dir}/speechlm_${task}_alimeeting_${config_name}" \
    --expdir "${exp_dir}" \
    --tokenizer "diar_tokenizer" \
    --output_format "event" \
    --check_overlap_sad true \
